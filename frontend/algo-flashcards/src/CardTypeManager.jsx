...
export default function CardTypeManager({ open, onClose }) {
  const [form, setForm] = useState({ name: '', description: '', fields: '' });
  const [error, setError] = useState(null);
  const [success, setSuccess] = useState(null);
  const [layout, setLayout] = useState({ front: [], back: [] });
  ...
  useEffect(() => {
    ...
    setLayout(l => {
      ...
      return { front: [...front, ...unassigned], back, preview: l.preview ? l.preview.filter(f => list.includes(f)) : [] };
    });
  ...
  const handleCreate = async e => {
    ...
    // Layout must cover all fields
    const allLayoutFields = [...layout.front, ...layout.back];
    if (fieldsList.some(f => !allLayoutFields.includes(f))) {
      setError('All fields must be assigned to front or back.');
      return;
    }
    ...
    try {
      const res = await fetchWithAuth(`${import.meta.env.VITE_API_BASE_URL}/cardtypes/`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          fields: fieldsList,
          layout: { ...layout, preview: previewFields },
        }),
      });
      if (!res.ok) {
        let errMsg = 'Could not create card type';
        try {
      ...
      setLayout({ front: [], back: [] });
      setFieldsList([]);
      setSuccess('Card type created!');
    } catch {
      setError('Could not create card type');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full relative">
        <button
          ...
          aria-label="Close"
        >×</button>
        <h2 className="text-2xl font-semibold mb-4">Create Card Type</h2>
        <form onSubmit={handleCreate} className="space-y-3">
          <label className="block">
            Name
            <input name="name" value={form.name} onChange={handleChange} required className="w-full border rounded px-2 py-1 mt-1" />
          </label>
          <label className="block">
            Description
          ...
          <label className="block">
            ...
            <input name="fields" value={form.fields} onChange={handleChange} placeholder="problem,difficulty,solution" required className="w-full border rounded px-2 py-1 mt-1" />
          </label>
          {/* Layout editor */}
          {fieldsList.length > 0 && (
            <>
              ...
              <div className="my-4">
                <div className="font-semibold mb-1">Deck Preview Fields</div>
                ...
                <div className="text-xs text-gray-500 mt-2">Select which fields are shown for each card in the deck overview.</div>
              </div>
            </>
          )}
          <button type="submit" className="rounded bg-green-600 text-white px-4 py-2 mt-2">Create</button>
